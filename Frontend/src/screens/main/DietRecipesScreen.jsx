...
export default function DietRecipesScreen({ 
  ...
  const [dailyPlan, setDailyPlan] = useState(() => {
    ...
    return getDynamicPalengkePlan(selectedLocation || 'Cebu City', targetCalories || 2000);
  });
  ...
  useEffect(() => {
    ...
    const loadCachedOrFetchMeals = async () => {
      ...
      try {
        ...
        if (__DEV__) console.log("MEAL RECOMMENDATION SILENT FETCH ERROR:", err);
      } finally {
        ...
        {activeDietTab === 'PLAN' ? (
          ...
          <View style={styles.dailyPlanSection}>
            ...
            {(() => {
              ...
              const currentConsumedCal = parseFloat(dailyNutrition?.consumedCalories || 0);
              const currentConsumedProt = parseFloat(dailyNutrition?.protein?.current || 0);
              const currentConsumedCarb = parseFloat(dailyNutrition?.carbs?.current || 0);
              const currentConsumedFat = parseFloat(dailyNutrition?.fats?.current || 0);

              const remCal = Math.max(100, totalTargetCal - currentConsumedCal);
              const remProt = Math.max(10, totalTargetProt - currentConsumedProt);
              const remCarb = Math.max(10, totalTargetCarb - currentConsumedCarb);
              ...
              planList.forEach((meal, idx) => {
                ...
                const isLogged = loggedMeals.some(m => {
                  if (typeof m === 'object' && m !== null) {
                    return String(m.id) === mealId || String(m.mealType || m.category).toLowerCase() === String(mealCat).toLowerCase();
                  }
              ...
              return (
                <View style={styles.timelineContainer}>
                  {hasAnyLoggedMeal && (
                    <View style={{
                      backgroundColor: isDarkMode ? 'rgba(16, 185, 129, 0.14)' : 'rgba(16, 185, 129, 0.09)',
                      ...
                      <Sparkles color="#10B981" size={16} style={{ marginRight: 8 }} />
                      <Text style={{ fontSize: 12, fontWeight: '700', color: isDarkMode ? '#6EE7B7' : '#047857', flex: 1 }}>
                        Smart Rebalancing Active: Remaining AI meals automatically adjusted to keep you on track for your {Math.round(totalTargetCal)} kcal goal!
                      </Text>
                    </View>
                  )}
                  ...
                  {isGeneratingMealPlan && planList.length === 0 ? (
                    ...
                    planList.map((meal, index) => {
                      ...
                      let catName = translateMealCategory(meal?.mealType || 'Meal', language);
                      let displayTitle = meal?.title || 'Healthy Meal';
                      let displayCalories = meal?.calories || 0;
                      let displayProtein = meal?.proteinNum || parseInt(meal?.protein) || 0;
                      let displayCarbs = meal?.carbsNum || parseInt(meal?.carbs) || 0;
                      let displayFats = meal?.fatsNum || parseInt(meal?.fats) || 0;
                      let badgeLabel = catName;

                      if (isLogged) {
                        if (typeof slotLoggedMeal === 'object' && slotLoggedMeal !== null) {
                          displayTitle = slotLoggedMeal.name || slotLoggedMeal.title || meal?.title;
                          displayCalories = slotLoggedMeal.calories || meal?.calories || 0;
                        ...
                        const weight = (DEFAULT_SLOT_WEIGHTS[mealCat] || 0.25) / unloggedWeightSum;
                      ...
                      return (
                        <View key={mealId} style={styles.timelineItem}>
                          ...
                          >
                            ...
                            <Text style={[styles.timelineTitle, isLogged && { color: '#64748B' }]}>{displayTitle}</Text>
                            <View style={styles.timelineFooter}>
                              <View style={{ flex: 1, paddingRight: 8 }}>
                                <Text style={styles.timelineMacroText}>{displayCalories} kcal • {displayProtein}g protein</Text>
                                <TouchableOpacity 
                      ...
                      <>
                        ...
                        <View style={styles.timelineList}>
                          {todayPlan.map((mealItem, idx) => {
                            ...
                            const translatedCat = translateMealCategory(rawCat, language);
